...
public class ServerRequestFormer {
	...
	public String finishEnemyPlayer() {
		return createServerRequest("fight_finish", null);
	}
	...
	public String registerDuelRequest(int duration, int timeout) {
		
		HashMap<String, String> param = new HashMap<String, String>();
		param.put("duration", Integer.toString(duration));
		param.put("timeout", Integer.toString(timeout));

		return createServerRequest("location_register_duel_application", param);
	}
	...
	public String fightAttack(Object... objects) {
		...
		AttackTarget attackType = null;
		BlockTarget blockType_1 = null;
		BlockTarget blockType_2 = null;
		
		for (Object object : objects) {
			if (object instanceof AttackTarget) {
				if (!firstAttackTypeFound) {
					attackType = (AttackTarget) object;
					firstAttackTypeFound = true;
				}
			} else if (object instanceof BlockTarget) {
				if (!firstBlockTypeFound) {
					blockType_1 = (BlockTarget) object;
					firstBlockTypeFound = true;
				} else {
					if (!secondBlockTypeFound) {
						blockType_2 = (BlockTarget) object;
